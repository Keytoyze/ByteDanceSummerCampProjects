...
public class MainActivity extends AppCompatActivity {
    ...
    public void testJSONObject(View view) {
        // DONE 1: Parse CAT_JSON using JSONObject
        String id = "";
        try {
    ...
    public void testGson(View view) {
        // DONE 2: Parse CAT_JSON using Gson
        String url = getGson().fromJson(CAT_JSON, Cat.class).getUrl();
        ImageHelper.displayWebImage(url, ivOut);
    }

    public void testHttpURLConnectionSync(View view) {
        // DONE 4: Fix crash of NetworkOnMainThreadException
        new Thread(new Runnable() {
            @Override
            public void run() {
                final String s = NetworkUtils.getResponseWithHttpURLConnection(
                        ICatService.HOST + ICatService.PATH);
    ...
    public void testRetrofitSync(View view) {
        // DONE 5: Making request in retrofit
        getCatService().randomCat(1).enqueue(new Callback<List<Cat>>() {
            @Override
            public void onResponse(Call<List<Cat>> call, final Response<List<Cat>> response) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        tvOut.setText(response.body().get(0).toString());
                    }
                });
            }
    ...
    public void testUpdateUI(View view) {
        // DONE 6: Fix crash of CalledFromWrongThreadException
        new Thread() {
            @Override
            public void run() {

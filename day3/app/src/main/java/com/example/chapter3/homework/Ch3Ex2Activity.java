...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        // FIXME: 回答：因为ofInt产生的是连续的颜色整数值变化，而不是连续的颜色变化。使用ofInt之后将会产生跳跃的颜色值。
        // FIXME: 如果要使用ofInt来产生连续变化的颜色，可以对颜色的三个分量（R / G / B)分别使用ofInt，之后再合成为颜色。
        ...
        // 对 target 控件的大小进行缩放，从 1 到 2 循环
        ObjectAnimator animatorScale = ObjectAnimator.ofPropertyValuesHolder(target,
                PropertyValuesHolder.ofFloat("scaleX", 1f, 2f),
                PropertyValuesHolder.ofFloat("scaleY", 1f, 2f)
        );
        animatorScale.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animatorScale.setRepeatCount(ObjectAnimator.INFINITE);
        animatorScale.setRepeatMode(ObjectAnimator.REVERSE);

        // 对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ...
        animatorSet.playTogether(animator1, animatorScale, animatorAlpha);
        animatorSet.start();
    }
}
